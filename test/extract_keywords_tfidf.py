...
# ---------- MERGE ---------
df_am_filtered = df_keywords_am[
    (df_keywords_am["score"] > 0.05)
    & (~df_keywords_am["keyword"].isin(df_keywords_live["keyword"]))
]
...
combined = pd.concat(
    [
        df_keywords_am.reset_index(drop=True),
        blank_col.reset_index(drop=True),
        df_keywords_live.reset_index(drop=True),
        blank_col.reset_index(drop=True),
        df_am_filtered.reset_index(drop=True),
    ],
    axis=1,
    ignore_index=False,
)
